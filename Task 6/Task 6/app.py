...
def upload():
    ...
    for (x, y, w, h) in faces:
        ...
        eye_distance = np.linalg.norm(np.array(eyes[0][:2]) - np.array(eyes[1][:2])) if len(eyes) >= 2 else 0
    ...
    result_path = os.path.join(UPLOAD_FOLDER, "result_" + file.filename).replace("\\", "/")
    cv2.imwrite(result_path, img)
    print("Saved image at:", result_path)
